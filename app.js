    ...
    Timer = setInterval(()=>{
        ...
        if(secT < 0){
            ...
            if(minT < 0){
                ...
                retryButton[0].style.transform = "scale(0, 0)"
                Input.disabled = true;
                createReportCard()
                setTimeout(()=>{
...
retryButton[1].addEventListener("click", ()=>{
    timer.disabled = false;
    reportCard.style.transform = "scale(0, 0)"
    setTimeout(()=>{
        timer.style.transform = "scale(1, 1)"
        retryButton[0].style.transform = "scale(1, 1)"
    }, 500)
    resetValues();
})

retryButton[0].addEventListener("click", ()=>{
    timer.disabled = false;
    resetValues();
})
